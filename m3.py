...
if __name__ == "__main__":
    ...
    raw_data = spark.read.format("csv").option("header","true").option("inferSchema", "true").load(r"/temp/input/adultdata_cleaned.csv")
    raw_data2 = spark.read.format("csv").option("header","true").option("inferSchema", "true").load(r"/temp/input/adulttest_cleaned.csv")
    ...
    #logistic regression
    #lr = LogisticRegression(labelCol="income", featuresCol="Scaled_features",maxIter=20)
    #dt = DecisionTreeClassifier(labelCol="income", featuresCol="Scaled_features")
    rf = RandomForestClassifier(labelCol="income", featuresCol="Scaled_features", numTrees=10)
    model=rf.fit(train)
    ...
    print('-----------------------------------------------')
    predict_train.select("income","rawPrediction","prediction","probability").show(40)
    predict_test.select("income","rawPrediction","prediction","probability").show(40)
    print('-----------------------------------------------')
    ...
    label_temp = np.array(predict_test.select('income').collect())
